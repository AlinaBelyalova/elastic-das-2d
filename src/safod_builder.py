...
class SafodBuildMetadata:
    ...
    x_fault_tie_m: float  # ИЗМЕНЕНО: четко указываем, что это координата разлома
    z_tie_m: float
    fault_x_offset_m: float  # ДОБАВЛЕНО: для воспроизводимости
...
def build_safod_model(
    ...
    pad_z_m: float = 500.0,
    z_tie_m: float = 2100.0,
    fault_x_offset_m: float = 0.0,  # ДОБАВЛЕНО
    fault_dip_deg: float = 82.0,
    fault_dip_sign: float = -1.0,
    fault_width_m: float = 150.0,
    velocity_drop_fraction: float = 0.30,
    ...
    # ИЗМЕНЕНО: логика вычисления координаты разлома
    x_cable_tie_m = float(np.interp(z_tie_m, z_cable, x_cable))
    x_fault_tie_m = x_cable_tie_m + fault_x_offset_m
    ...
    # ИЗМЕНЕНО: теперь опираемся на x_fault_tie_m
    dist_to_fault = (
        X - x_fault_tie_m - m_fault * (Z - z_tie_m)
    ) / np.sqrt(1.0 + m_fault**2)

    x_fault_line = x_fault_tie_m + m_fault * (grid.z - z_tie_m)
    ...
    metadata = SafodBuildMetadata(
        geom_file=geom_file,
        build_initial_model=build_initial_model,
        x_fault_tie_m=x_fault_tie_m,  # ИЗМЕНЕНО
        z_tie_m=z_tie_m,
        fault_x_offset_m=fault_x_offset_m,  # ДОБАВЛЕНО
        fault_dip_deg=fault_dip_deg,
        fault_dip_sign=fault_dip_sign,
        fault_width_m=None if build_initial_model else fault_width_m,
        left_block_name=left_block_name,
